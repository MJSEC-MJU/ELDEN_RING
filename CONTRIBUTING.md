...
**앱 코드**는 `services/`, **K8s 매니페스트**는 `kubernetes/environments/` 에 분리합니다.

```

├── services/                            ← 앱 코드 (Phase별)
│   ├── runtime-defense/                     # Phase 1 (이주오)
│   │   ├── src/
│   │   │   └── main.py
│   │   ├── tests/
│   │   ├── Dockerfile
...
│       └── ...
├── kubernetes/environments/             ← K8s 매니페스트만
│   ├── production/
│   │   ├── deployment.yaml                  # target-app (보호 대상)
│   │   └── runtime-defense.yaml             # Phase 1 Deployment + Service
│   ├── secure-coding/
│   │   └── deployment.yaml                  # Phase 2 Deployment + Service
│   ├── staging/
│   │   └── deployment.yaml                  # Phase 3 + target-app staging
│   └── governance/
│       └── deployment.yaml                  # Phase 4 Deployment + Service
```

### CI 자동화

`dev` 브랜치에 머지하면 **변경된 Phase만** 자동으로 빌드/배포됩니다.

| 변경 감지 경로 | 빌드되는 이미지 | 배포 NS |
|---|---|---|
| `services/runtime-defense/**` | `eldenring/runtime-defense:dev-xxx` | `elden-production` |
| `services/secure-coding/**` | `eldenring/secure-coding:dev-xxx` | `elden-secure-coding` |
| `services/recovery-assurance/**` | `eldenring/recovery-assurance:dev-xxx` | `elden-staging` |
| `services/governance/**` | `eldenring/governance:dev-xxx` | `elden-governance` |

### 작업 위치 정리

| 작업 | 위치 |
|---|---|
| Python/Go 코드, Dockerfile, 테스트 | `services/<내 Phase>/` |
| K8s Deployment, Service YAML | `kubernetes/environments/<내 NS>/` |

---

## Redis 메시지 브로커 (Phase 간 통신)

Phase 간 JSON 컨텍스트 전달은 Redis Pub/Sub를 사용합니다.
...
Port: 6379
인증: 없음 (클러스터 내부 전용, NetworkPolicy로 보호)
```
...
| 채널 | 발행자 | 구독자 | 데이터 |
|---|---|---|---|
| `elden:phase2:context` | Phase 1 | Phase 2 | 구조화 컨텍스트 (event_id, CWE, source_location) |
| `elden:phase3:validate` | Phase 2 | Phase 3 | 후보 이미지 (patch_id, candidate_image) |
| `elden:phase4:promote` | Phase 3 | Phase 4 | 검증 완료 (validated_image, validation_result) |
| `elden:phase2:retry` | Phase 3 | Phase 2 | 검증 실패 → 재수정 요청 |
...
r = redis.Redis(host='redis-master.elden-monitoring', port=6379)
...
r.publish('elden:phase2:context', json.dumps(context))
...
for message in pubsub.listen():
    if message['type'] == 'message':
        ...
        print(f"Received: {data['event_id']}")
```
...
rdb := redis.NewClient(&redis.Options{
    Addr: "redis-master.elden-monitoring:6379",
})

// 발행
rdb.Publish(ctx, "elden:phase2:context", jsonData)

// 구독
sub := rdb.Subscribe(ctx, "elden:phase2:context")
ch := sub.Channel()
for msg := range ch {
...
```bash
kubectl run redis-test --rm -it --image=redis:alpine -n elden-production -- \
  redis-cli -h redis-master.elden-monitoring ping
# 응답: PONG
```
